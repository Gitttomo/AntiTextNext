-- Integrate Purchase Approval with Atomicity RPC
-- Updates the submit_purchase_request function to handle pending_approval and request history
...
BEGIN
  ...
  -- Keep item status as 'available' - DO NOT set it to transaction_pending yet.
  -- But we should release the lock since the transaction is created (pending_approval).
  UPDATE public.items
  SET
    locked_by = NULL,
    locked_until = NULL
